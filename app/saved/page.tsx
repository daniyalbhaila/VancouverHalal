...
export default function SavedPage() {
    ...
    if (loading) {
        return (
            <div className="flex flex-col items-center justify-center min-h-screen bg-[var(--bg-base)] transition-colors">
        ...
        <div className="min-h-screen bg-[var(--bg-base)] pb-32 transition-colors">
            ...
            <div className="sticky top-0 z-40 bg-[var(--glass-bg)] backdrop-blur-md border-b border-[var(--glass-border)] p-4 pt-safe top-safe transition-colors">
                <div className="flex items-center gap-3">
                    ...
                    <span className="ml-auto text-xs font-semibold text-[var(--text-secondary)] bg-[var(--glass-bg)] px-2 py-1 rounded-full">{favorites.length}</span>
                </div>
            </div>
            ...
            <div className="p-4 pt-6 max-w-md mx-auto space-y-6">
                {savedRestaurants.length > 0 ? (
                    ...
                    <div className="flex flex-col items-center justify-center py-20 text-center">
                        ...
                        <p className="text-[var(--text-secondary)] mb-8 max-w-[250px]">Start swiping right on restaurants to add them to your collection.</p>
                        <Link href="/swipe" className="px-6 py-3 bg-[var(--text-primary)] text-[var(--bg-base)] rounded-full font-bold text-sm shadow-lg">
                            Go to Swipe Deck
                        </Link>
                    </div>
                )}
